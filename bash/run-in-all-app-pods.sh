...
pods=$(kubectl get pods -n nvidia-device-plugin -l app=gpu-test -o jsonpath='{.items[*].metadata.name}')

# Loop through each pod
for pod in $pods; do
    echo "Processing pod: $pod"

    # Create main.py file in the pod
    kubectl exec -n nvidia-device-plugin $pod -- bash -c "cat << EOF > main.py
import argparse
import logging
import sys
import time
